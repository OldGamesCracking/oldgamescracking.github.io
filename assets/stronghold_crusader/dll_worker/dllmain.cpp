...
#define IAT_START           0x00584000
#define IAT_END             0x005842D8
#define USER_MODULES_END    0x10000000
#define RAW_DATA_ADDRESS    0x00ADA20C
#define NUM_NANOMITES       100
...
/// <param name="to"></param>
/// <returns></returns>
BOOL IsAPathToAddress(const BYTE *const buffer, SIZE_T bufferLen, SIZE_T to)
{
    for (SIZE_T b = 0; b < bufferLen - 6; b++)
    {
        if (buffer[b] == OP_JMP_NEAR || buffer[b] == OP_JE_NEAR || buffer[b] == OP_JNE_NEAR)
        {
            BYTE jmpOffset = buffer[b + 1];
            SIZE_T destination = b + 2 + jmpOffset;

            if (destination == to && jmpOffset != (BYTE)(-2))
            {
                return TRUE;
            }
        }

        if (*(WORD*)&buffer[b] == OP_JE_FAR || *(WORD*)&buffer[b] == OP_JNE_FAR || *(WORD*)&buffer[b] == OP_JLE_FAR || *(WORD*)&buffer[b] == OP_JG_FAR)
        {
            DWORD jmpOffset = *(DWORD*)&buffer[b + 2];
            SIZE_T destination = b + 6 + jmpOffset;

            if (destination == to && jmpOffset != (DWORD)(-6))
            {
                return TRUE;
            }
        }
...
void Run(DEBUG_EVENT *const event, PROCESS_DATA_t *const data)
{
    ...
    while (true)
    {
        ...
        {
            logger.Log("Unexpected dwDebugEventCode: 0x%08X (continueable)", eventCode);

            const DWORD startAddress = (DWORD)event->u.CreateThread.lpStartAddress;

            logger.Log("startAddress: 0x%08X", startAddress);
...
/// <summary>
/// Performs single steps until the byte at [Eip] matches value
/// </summary>
/// <param name="event"></param>
/// <param name="data"></param>
...
/// <returns></returns>
DWORD GetProc_Generic(DEBUG_EVENT *const event, PROCESS_DATA_t *const data, DWORD startFrom)
{
    SetEip(data, startFrom);
    SingleStepUntil8(event, data, OP_PUSHFD);
    SingleStep(event, data);
...
void FixNanomites(PROCESS_DATA_t* const processData)
{
    ...
    NANOMITE_RAW_DATA_t *rawData = (NANOMITE_RAW_DATA_t*)RAW_DATA_ADDRESS;
...
BOOL FixCalls_Generic(DEBUG_EVENT *const event, PROCESS_DATA_t *const data, BYTE *const pattern, SIZE_T patternLen, BOOL dummyCall = FALSE)
{
    ...
    logger.Log("\tpatternLen: %d", patternLen);
    logger.Log("\tdummyCall: %s", dummyCall ? "TRUE" : "FALSE");
    ...
    ReadProcessMemory(data->hProcess, (LPVOID)IAT_START, &iat[0], iatSize, &bytesRead);
...
BOOL FixRelayedUserCodeCalls(DEBUG_EVENT* const event, PROCESS_DATA_t *const data)
{
...
BOOL FixFarJMPCalls(DEBUG_EVENT* const event, PROCESS_DATA_t* const data)
{
    ...
    for (SIZE_T b = 0; b < data->TextSectionSize - 5; b++)
    {
        if (textSection[b] == OP_JMP_REL)
        {
            ...
            WORD checkVal;
            ReadProcessMemory(data->hProcess, (LPVOID)jmpTo, &checkVal, sizeof(checkVal), &bytesRead);
            ...
            DWORD proc = GetProc_Generic(event, data, jmpAt);

            /* Delete address or we get an infinite loop */
            BYTE call[] = { 0xFF, 0x15, 0x00, 0x00 , 0x00 , 0x00 };
            WriteMemory(data, (LPVOID)jmpAt, &call[0], sizeof(call));

            call_list_add(data->calls_Intermodular, jmpAt, proc, 2);
        }
    }
...
void FixIAT(PROCESS_DATA_t *const data)
{
    ...
    ReadProcessMemory(data->hProcess, (LPVOID)IAT_START, &IAT[0], IATSize, &numBytes);
    ...
    while (head != NULL)
    {
        ...
        WriteProcessMemory(data->hProcess, (LPVOID)(head->CallAt + head->InstructionLen), &thunkReal, sizeof(DWORD), &numBytes);
    ...
    /* Write back IAT */
    WriteProcessMemory(data->hProcess, (LPVOID)IAT_START, &IAT[0], IATSize, &numBytes);
