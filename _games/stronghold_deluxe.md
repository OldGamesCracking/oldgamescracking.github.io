...
If we run the game normally in the VM, everything works fine, but as soon as we let it run in the debugger, it crashes. So it looks like since this version an improved anti debugging scheme was used. Let's track it down. Interestingly, ProcMon reveals that the game seems to boot up fine, but then somewhere crashes down the line. I tried to use ScyllaHide this time but it either crashed or the program seemed to run in some infinite loop.<br>
...
- The game.exe creates another process, let's call it debugger.exe and passes it's PID to it.
- The game.exe creates two events. One called ALT_{PID} and the other BLT_{PID}, both un-set initially.
- The game.exe sets the event ALT_{PID} and then waits for BLT_{PID} to be set.
- The debugger.exe detects that ALT_{PID} has been set and now tries to attach to game.exe as a debugger.
- Since we already debug game.exe, the debugger.exe can not attach and thus will never set BLT_{PID}.
...
The plan I had now was to just let the game run until it creates the event, then steal the handle, jump to _SetEvent_ and set the event myself :) The script for that looks like the following:<br>
...
That worked surprisingly well. One could probably also have altered the parameters passed to _WaitForSingleObject_ or returned prematurely from that. Time to fix the IAT. I used the following settings for [my SafeDisc script](https://github.com/OldGamesCracking/oldgamescracking.github.io/blob/main/assets/safedisc/safedisc_import_fixer.txt):
...
I then tried to run the game but only a few instructions into the game we land on this bit:
...
The setup function simply saves EAX and ECX and calls the inner function. The inner function adds a constant offset to the return value and uses that as a lookup to jump to something we have seen before countless times (if you've read the previous articles on SafeDisc):
...
- For each of those, resolve the original address via the well known methods described before
- Patch the CALL to call the original location
...
- Attach your debugger to the game's debugger.exe and set a breakpoint on _WaitForDebugEvent_
...
If all original bytes are figured out, we can continue where we left off, patch the original bytes and the game is hopefully freed of SafeDisc.<br>

As I've said. This should work well, but what if the debugger.exe has also some anti-debugging mechanisms? Maybe there is a more stealthy approach - one that makes us also use a real programming language :) Well, ask no more, I present you: DLL-Injections combined with API hooking/detouring. If you want more background info on that topic, search the net, there is lots of information out there and also some quite advanced libraries. But for general understanding and simplicity, we will stick to the bare minimum and write our own hooking library, it's actually quite simple.<br>
...
The actual steps are a bit more finicky, but I will go through them one by one. I have also recreated w4kfu's timing diagram to represent more my solution:
...
The source code of the injector can be found [here](assets/simple_injector).<br>
...
Another detail I did mention yet is the fact that some Nanomites are not restored instantly (remember case a) and b) from before?). Luckily SafeDisc will spit them out after the 10th exception. So a bit of checking if the Nanomite was restored or not is needed.<br>

Also, have you already thought about the problem on how to actually trigger the Nanomite to let it get restored? If we would simply point EIP there, we would have no control over the program flow afterwards. At least not easily. One solution could be to patch the call to _SetThreadContext_ so we get reset to a known location after every reset. But there is actually a much simpler solution: Create a new thread on the Nanomite and let it get killed by our dll\_worker.dll ;)<br>
...
With these few details you should be able to read the sourcecode on you own ;) Just a short description on what's going on in detail:

* The DLL is loaded and a worker thread is created since we should [not do anything scary in the DllMain](https://devblogs.microsoft.com/oldnewthing/20040127-00/?p=40873)
* In the worker, the console is spawned and the hooks to _CreateProcessA_, _CreateEventA_ and _WaitForSingleObject_ are set up
...
* In the thread a debug-log file (worker\_log.txt) is created and a file that will contain the restored bytes (restored\_bytes.1337), this file is in the patchfile-format of x32dbg so we can load it easily.
* Also the following functions are hooked: _WaitForDebugEvent_, _SetThreadContext_, _WriteProcessMemory_, _ContinueDebugEvent_
* First, _WaitForDebugEvent_ should trigger. If an exception is detected, the thread ID is noted down and _threadContextSet_ and _processMemoryWritten_ is reset.
* If _SetThreadContext_ is triggered, _threadContextSet_ is set to true.
* If _WriteProcessMemory_ is triggered, _processMemoryWritten_ is set to true and and also the old and new bytes are written to the patchfile
...
We now have everything to fully fix the game. The patchfile is in the %temp% folder if you haven't already figured out. I found out that it does not seem to matter if we first apply the patches and then let the IAT-fixer script run or the other way round. The only thing that does seem to matter is that we need to rename the gamefile back to _Stronghold.exe_. If we keep the \_dump\_SCY suffix that Scilla adds, the game crashes as it does seem to perform some lookups (in the Registry?) based on the name.<br>
